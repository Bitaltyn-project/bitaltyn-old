...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitaltyn: starting difficulty is 1 / 2^12
...
{
    // Bitaltyn: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 10 * 30; // Bitaltyn: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Bitaltyn: 30 seconds
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitaltyn: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bitaltyn: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitaltynMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitaltynMiner(CWallet *pwallet)
{
    printf("BitaltynMiner started\n");
    ...
    try { loop {
        ...
        printf("Running BitaltynMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitaltynMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitaltynMiner, pwallet));
}
